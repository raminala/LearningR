...
eu_stocks <- as.ts(read.csv("eu_stocks.csv"), start = c(1991, 130), end = c(1998, 169), frequency=260)
...
eu_percentreturns <- as.ts(read.csv("eu_percentreturns.csv"), start = c(1991, 130), end = c(1998, 169), frequency=260)

DAX <- ts(read.csv("DAX.csv"), start = 1871 , end = 1970, frequency = 1)
FTSE <- ts(read.csv("FTSE.csv"), start = 1871 , end = 1970, frequency = 1)

DAX_logreturns <- ts(read.csv("DAX_logreturns.csv"), start = 1871 , end = 1970, frequency = 1)

FTSE_logreturns <- ts(read.csv("FTSE_logreturns.csv"), start = 1871 , end = 1970, frequency = 1)

x1 <- ts(read.csv("x1.csv"))
...
## 3-2 Asset prices vs. asset returns
...
## 3-3 Characteristics of financial time series
...
## 3-6 Calculating sample covariances and correlations
...
## 3-9 Calculating autocorrelations
...
Luckily, the acf() command provides a shortcut. Applying
...
## 3-10 The autocorrelation function
...
function will estimate all autocorrelations from 0, 1, 2,..., up to the value specified by the argument lag.max. In the previous exercise, you focused on the lag-1 autocorrelation by setting the lag.max argument to 1.
...
## 3-11 Visualizing the autocorrelation function
...
## 4-2 Simulate the autoregressive model
...
## 4-3 Estimate the autocorrelation function (ACF) for an autoregression
...
## 4-5 Compare the random walk (RW) and autoregressive (AR) models
...
## 4-7 Estimate the autoregressive (AR) model
...
## 4-8 Simple forecasts from an estimated AR model
...
## 5-2 Simulate the simple moving average model
...
## 5-3 Estimate the autocorrelation function (ACF) for a moving average
...
## 5-5 Estimate the simple moving average model
...
## 5-6 Simple forecasts from an estimated MA model
